    ...
    <div class="container-p" v-if="menu_data">
        ...
        <div class="flex-adaptive footer-main">
          <div class="footer-menu">
            <ul class="item">
              <li v-for="(item, key) in menu_data.menus" :key="key" v-if="!item.subsections">
                <nuxt-link v-if="!item.subsections" :to="localePath(item.url)" active-class="active">
                  {{item.name}}
                </nuxt-link>
              </li>
              <!-- <li>
                <nuxt-link :to="localePath('/family')" active-class="active">
                  {{ $t('menu[41]') }}
                </nuxt-link>
              </li> -->
            </ul>
          ...
          <div class="footer-info">
            <div>
              ...
              <div class="text-x3 fw-6"><a :href="'tel:'+menu_data.phoneTwo">{{menu_data.phoneTwo | spacePhone}}</a></div>
            </div>
            <br>
            <div>
              <p>{{menu_data.phone_title}}</p>
              <h4 class="m-t-0"><a :href="'tel:'+menu_data.phone">{{menu_data.phone | spacePhone}}</a></h4>
            </div>
            <br>
            <div v-if="menu_data.helpline">
              <p>{{menu_data.helpline_title}}</p>
              <h4 class="m-t-0"><a :href="'tel:'+menu_data.helpline">{{menu_data.helpline | spacePhone}}</a></h4>
            </div>
            <br><br>
      ...
      <div class="footer-feedback clearfix">
        <div class="flex-adaptive">
          <div class="col-md-8">
            <p><small>{{menu_data.footer.text[1]}}</small></p>
            <br><br>
            <ul class="list-inline color-gray-4 font-size-1 hide">
              <li><a href="">{{ $t('footer[0]') }}</a></li>
              <li><a href="">{{ $t('footer[1]') }}</a></li>
            </ul>
          </div>
          <div class="col-md-4 text-right-md hide">
            <span class="btn-opacity color-white">
              <nuxt-link :to="localePath('/feedback')">{{ $t('btns.type_4') }}</nuxt-link>
            </span>
            <br><br>
